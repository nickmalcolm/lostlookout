class Cartographer::Gmarker
  #include Reloadable
  attr_accessor :name, :marker_type, :highlight, :icon, :position, :click, :info_window, :info_window_url, :info_window_auto_pop, :map, :min_zoom, :max_zoom, :dblclick, :draggable
  ...
  def initialize(options = {})
    ...
    @info_window_auto_pop = options[:info_window_auto_pop] || false #true if the info window auto shows
    @map = options[:map]
  ...
  def to_js(marker_mgr_flag = false, marker_clusterer_flag = false)
    ...
    if @info_window_auto_pop
      script << "var fetched_content = cartographer_ajax_fetch_url(\"#{info_window_url}\");
                #{@map.shared_info_window.name}.setContent(fetched_content);
                #{@map.shared_info_window.name}.open(#{@map.dom_id},#{name});"
    end
  ...
  def zoom_link(link_text = 'Zoom on map')
    "<p><a href='#' onClick='#{@map.dom_id}.setCenter(new google.maps.LatLng(#{@position.first}, #{@position.last}), 8); return false;'>#{link_text}</a></p>"
  end
